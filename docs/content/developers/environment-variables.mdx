...
| Variable              | Required   | Description                               | Example                        |
| --------------------- | ---------- | ----------------------------------------- | ------------------------------ |
| `EMAIL_PROVIDER`      | No         | Email provider: `brevo`, `smtp`, or `ses` | `ses`                          |
...
| Variable                    | Required | Description                               | Example     |
| --------------------------- | -------- | ----------------------------------------- | ----------- |
| `AWS_SES_REGION`            | SES only | AWS region where SES is configured        | `us-east-1` |
| `AWS_SES_ACCESS_KEY_ID`     | No       | AWS access key (uses IAM role if omitted) | `AKIA...`   |
| `AWS_SES_SECRET_ACCESS_KEY` | No       | AWS secret key (uses IAM role if omitted) | `wJalr...`  |
| `AWS_SES_CONFIGURATION_SET` | No       | SES configuration set for tracking        | `my-config` |
...
<Callout type="info">
  On EC2/Lambda/ECS, you can omit the access key and secret to use IAM roles
  automatically.
</Callout>

...
Auktiva supports three email providers: **Brevo** (cloud service), **SMTP** (any SMTP server), and **Amazon SES** (AWS native).

### Common Settings

```bash
# Provider selection: "brevo", "smtp", or "ses"
...
Use any SMTP server (Gmail, Mailgun, Amazon SES SMTP, self-hosted, etc.).
...
### Option 3: Amazon SES (API)

Use the AWS SDK for native SES integration. This is the recommended approach for AWS deployments.

```bash
EMAIL_PROVIDER="ses"
AWS_SES_REGION="us-east-1"
AWS_SES_ACCESS_KEY_ID="AKIAIOSFODNN7EXAMPLE"  # Optional with IAM roles
AWS_SES_SECRET_ACCESS_KEY="wJalrXUtnFEMI/..."  # Optional with IAM roles
AWS_SES_CONFIGURATION_SET="my-tracking-set"  # Optional
```

<Callout type="info">
  On EC2, Lambda, or ECS with an IAM role attached, you can omit the access key
  and secret. The SDK will automatically use the instance/task role credentials.
</Callout>

### Amazon SES: API vs SMTP

Amazon SES can be used in two ways with Auktiva. Here's a comparison to help you choose:

| Feature                | SES API (`EMAIL_PROVIDER="ses"`)    | SES SMTP (`EMAIL_PROVIDER="smtp"`)         |
| ---------------------- | ----------------------------------- | ------------------------------------------ |
| **Setup complexity**   | Simpler (just region + credentials) | Requires SMTP credentials from SES console |
| **IAM role support**   | Yes (automatic on EC2/Lambda/ECS)   | No (always needs explicit credentials)     |
| **Performance**        | Faster (direct API call)            | Slightly slower (SMTP handshake overhead)  |
| **Configuration sets** | Native support                      | Not supported                              |
| **Error messages**     | Detailed AWS error codes            | Generic SMTP errors                        |
| **Dependencies**       | Requires `@aws-sdk/client-ses`      | Uses existing `nodemailer`                 |
| **Portability**        | AWS-specific                        | Can switch to any SMTP provider            |

**Recommendation:**

- Use **SES API** (`EMAIL_PROVIDER="ses"`) if you're committed to AWS and want the best performance, IAM role support, or need configuration sets for tracking.
- Use **SES SMTP** (`EMAIL_PROVIDER="smtp"`) if you want flexibility to switch providers later or prefer a simpler, provider-agnostic setup.
...
| `EMAIL_PROVIDER`            | No         | -                     | Email provider (`brevo`, `smtp`, or `ses`) |
...
| `AWS_SES_REGION`            | SES only   | -                     | AWS region for SES                         |
| `AWS_SES_ACCESS_KEY_ID`     | No         | -                     | AWS access key (optional with IAM)         |
| `AWS_SES_SECRET_ACCESS_KEY` | No         | -                     | AWS secret key (optional with IAM)         |
| `AWS_SES_CONFIGURATION_SET` | No         | -                     | SES configuration set name                 |

...
Browser ← WebSocket ← Soketi/Pusher ← API (on data change)
                ↓
         Instant UI update

Fallback (Polling):
Browser → SWR (interval) → API Route → Database
                ↓
         Update UI on change
```

See [Realtime Features](/developers/realtime) for configuration details.

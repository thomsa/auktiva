...
- Server: `SOKETI_HOST="127.0.0.1"` → Next.js connects internally
- Browser: `NEXT_PUBLIC_SOKETI_HOST="auctions.example.com"` → Users connect via internet
  </Callout>
...
2. Verify `NEXT_PUBLIC_SOKETI_HOST` matches how users access your server
3. If using a reverse proxy, ensure WebSocket upgrade headers are passed
